...
import { MarketData as NightFuturesData } from "../../lib/market";

interface KospiNightFuturesShareCardProps {
  data: NightFuturesData;
  onClose: () => void;
}

export function KospiNightFuturesShareCard({
  data,
  onClose,
}: KospiNightFuturesShareCardProps) {
  ...
  const handleDownload = async () => {
    ...
    if (blob) {
      ...
      saveAs(blob, `hangon-kospi-night-futures.png`);
    }
  ...
  const handleWebShare = async () => {
    const blob = await getBlob();
    if (!blob) return;
    const file = new File([blob], "kospi-night-share.png", {
      type: "image/png",
    });
    try {
      await navigator.share({
        files: [file],
        title: "[HangOn] 코스피 야간선물 실시간 리포트",
        text: `코스피 야간선물 지수: ${data.value} (${data.changePercent})`,
      });
    } catch (err) {
                ...
                <div className="p-0 pt-2 flex-1 flex flex-col relative z-10">
                  <h2
                    className={`text-xl font-black italic mb-6 leading-tight ${
                      shareTheme === "dark" ? "text-white" : "text-slate-900"
                    }`}
                  >
                    KOSPI Night Futures
                  </h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {canShare ? (
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-3 bg-secondary text-foreground rounded-2xl font-black text-xs hover:bg-secondary/80 transition-all"
              >
                {isImageCopied ? (
