...
interface KospiFuturesShareCardProps {
  data: MarketData;
  onClose: () => void;
}

export function KospiFuturesShareCard({
  data,
  onClose,
}: KospiFuturesShareCardProps) {
  ...
  const handleDownload = async () => {
    ...
    if (blob) {
      ...
      saveAs(blob, `hangon-kospi-futures.png`);
    }
  ...
  const handleWebShare = async () => {
    const blob = await getBlob();
    if (!blob) return;
    const file = new File([blob], "kospi-share.png", { type: "image/png" });
    try {
      await navigator.share({
        files: [file],
        title: "[HangOn] 코스피 200 선물 실시간 리포트",
        text: `코스피 200 선물 지수: ${data.value} (${data.changePercent})`,
      });
    } catch (err) {
            ...
            <div className="scale-[0.85] xs:scale-90 sm:scale-100 origin-center shrink-0">
              ...
              >
                ...
                <div className="p-0 pt-2 flex-1 flex flex-col relative z-10">
                  ...
                  {/* Insight Marker */}
                  <div className="mt-6 pt-6 border-t border-tight border-dashed opacity-50 border-gray-400">
                    ...
                    >
                      국내 증시의 선행지표인 코스피 200 선물 실시간 흐름입니다.
                    </p>
                  </div>
